...
class CollectibleLight extends SpriteAnimationComponent
  ...
  void collidedWithPlayer({bool? isLightCollect, bool? isBulletCollect}) async {
    final collectedData = GameCollectibles()
        .getCollectibleData(KenumGameCollectibles.lightCollected);
    final gameStateProvider = Provider.of<GameStateProvider>(
        navigatorKey.currentContext!,
        listen: false);
    if (!collected) {
      if (isLightCollect == true) {
        gameStateProvider.addLightsCollected = 1;
      }
